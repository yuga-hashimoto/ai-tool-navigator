...
import { getActiveTimeSensitiveTiers, getBundleSavings, getPopularBundles, getBestValueBundles, getActiveUrgencySignals, calculateUrgencyConversionMetrics, trackConversionFunnel, recordPriceChange, generatePricingRecommendation } from '@/lib/dynamic-pricing/dynamicPricing';
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'tiers': {
        const tiers = id ? [getActiveTimeSensitiveTiers().find(t => t.id === id)].filter(Boolean) : getActiveTimeSensitiveTiers();
        return NextResponse.json({ success: true, data: tiers });
      }
      case 'bundles': {
        const bundles = id ? getBundleSavings(id) : { popular: getPopularBundles(), all: getBestValueBundles() };
      ...
      default:
        return NextResponse.json({
          success: true,
          data: { activeTiers: getActiveTimeSensitiveTiers(), popularBundles: getPopularBundles(), urgencySignals: getActiveUrgencySignals() }
        });
    }
  } catch (error) {
    console.error('Error fetching dynamic pricing data:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch dynamic pricing data' }, { status: 500 });
  }
}
  ...
  try {
    ...
    switch (action) {
      case 'track_conversion': {
        if (!tierId || !event) {
          return NextResponse.json({ success: false, error: 'Missing tierId or event' }, { status: 400 });
        }
      ...
      case 'record_price_change': {
        ...
        if (tierId === undefined || previousPrice === undefined || newPrice === undefined) {
          return NextResponse.json({ success: false, error: 'Missing required fields' }, { status: 400 });
        }
        const record = recordPriceChange(tierId, previousPrice, newPrice, reason || 'manual', conversionRate || 0, revenue || 0);
        return NextResponse.json({ success: true, data: record });
      }
      case 'simulate_purchase': {
        const tier = getActiveTimeSensitiveTiers().find(t => t.id === tierId);
        if (tier) { tier.currentPurchases += 1; }
        return NextResponse.json({ success: true, message: 'Purchase simulated', data: { currentPurchases: tier?.currentPurchases } });
      }
      default:
