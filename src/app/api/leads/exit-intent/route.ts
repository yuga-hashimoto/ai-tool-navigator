  ...
  try {
    ...
    try {
      if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
        ...
        console.log(`[EXIT INTENT LEAD] New lead appended to Google Sheets: ${emailHash} at ${new Date().toISOString()}`);
      } else {
        console.warn('GOOGLE_SERVICE_ACCOUNT_JSON not set, skipping Google Sheets append.');
        console.log(`[EXIT INTENT LEAD FALLBACK] New lead: ${emailHash} at ${new Date().toISOString()}`);
      }
    } catch (sheetsError) {
      ...
      if (errorMsg.includes('not set') || errorMsg.includes('not configured')) {
        console.warn(`[EXIT INTENT LEAD FALLBACK] Google Sheets not configured. Lead recorded locally: ${emailHash}`);
      } else {
