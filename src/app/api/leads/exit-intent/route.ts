  ...
  try {
    ...
    try {
      if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
        ...
        console.log(`[EXIT INTENT LEAD FALLBACK] Google Sheets not configured. Lead recorded locally: ${leadId}`);
      }
    } catch (sheetsError) {
      console.warn('Google Sheets append failed, falling back to local logging:', sheetsError);
      console.log(`[EXIT INTENT LEAD FALLBACK] Append failed. Lead recorded locally: ${leadId}`);
    }
