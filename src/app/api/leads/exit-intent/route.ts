  ...
  try {
    ...
    try {
      if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
        ...
        console.log(
          `[EXIT INTENT LEAD] Appended lead to Google Sheets: lead_${leadId.substring(5)}`,
        );
      } else {
        console.warn(
        ...
        console.log(
          `[EXIT INTENT LEAD FALLBACK] New lead captured locally: lead_${leadId.substring(5)}`,
        );
      }
    } catch (sheetsError) {
      console.warn(
        "Failed to append to Google Sheets, falling back to local storage:",
        sheetsError,
      );
      console.log(
        `[EXIT INTENT LEAD FALLBACK] New lead captured locally: lead_${leadId.substring(5)}`,
