...
export default function UrgencyBundleCard({
  bundle,
  variant = 'standard',
  onAddToCart,
  className = ''
}: UrgencyBundleCardProps) {
  const [isHovered, setIsHovered] = useState(false);

  // Featured variant styling
  const isFeatured = variant === 'featured';

  // Urgency type icons and colors
  const urgencyConfig = {
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className={`p-1 rounded ${isFeatured ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
              {urgency.icon}
            </span>
          ...
          <div className="flex flex-wrap gap-1 mb-3">
            ...
            {bundle.items.length > 2 && (
              <span className="text-xs text-gray-500">+{bundle.items.length - 2} more</span>
            )}
          </div>
        </div>
  ...
  return (
    <div 
      className={`
        ...
        ${isFeatured ? 'ring-2 ring-blue-500 shadow-xl scale-105' : 'shadow-lg'}
        hover:shadow-xl transition-all duration-300
        ${isFeatured ? 'transform hover:scale-110' : 'transform hover:-translate-y-1'}
        ${className}
      `}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Featured badge */}
      {isFeatured && (
        <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-center py-2 text-sm font-semibold z-10">
          ⭐ BEST VALUE - {bundle.savingsPercent}% OFF
        </div>
      )}

      {/* Popular badge */}
      {bundle.popular && !isFeatured && (
      ...
      <div className={`p-6 ${isFeatured ? 'pt-14' : ''} bg-gradient-to-br from-gray-50 to-white`}>
        <div className="flex items-center gap-2 mb-2">
          <span className={`p-1.5 rounded-lg ${isFeatured ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
            {urgency.icon}
          </span>
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className={`text-sm font-medium ${isFeatured ? 'text-blue-600' : 'text-orange-600'}`}>
            {bundle.urgencyMessage}
          </span>
        </div>
        ...
        {/* Stock warning */}
        {bundle.stockRemaining !== undefined && (
          <div className="mb-4">
            <StockRemaining 
              current={bundle.stockRemaining} 
              max={bundle.maxStock || 50}
              size="sm"
            />
          </div>
        )}
...
export function BundleList({
  ...
  const displayBundles = featuredOnly 
    ? bundles.filter(b => b.popular || b.bestValue)
    : bundles;
    ...
    <div className={`grid ${gridCols[columns]} gap-6`}>
      {displayBundles.map((bundle) => (
        <UrgencyBundleCard
          key={bundle.id}
          bundle={bundle}
          variant={bundle.popular || bundle.bestValue ? 'featured' : 'standard'}
